...
- 同一タブにエージェントがいれば送信（1つなら自動選択、複数あれば `vim.ui.select` で選択）
- 同一タブにおらず別タブにのみいる場合は、このタブに新規起動するか別タブのエージェントに送るかを選択
- エージェントが同一ワークスペースに存在しなければ自動でペイン分割して起動
- 送信後、送信先（ペインのタイトルと pane_id）を通知
...
| エージェント | 送信フォーマット | 備考 |
|-------------|-----------------|------|
| Claude Code | `@path#L1-5` | `@` 記法でファイル内容を自動展開 |
| Gemini CLI | `@path#L1-5` | `@` 記法でファイル内容を自動展開 |
| Kiro CLI | `path#L1-5` | パスをテキストとして送信（`@` なし） |

`agent_cmd` の値に応じて自動的にフォーマットが切り替わる。

上記以外のherdr対応エージェント（Codex, Copilot, Devin, Kimi 等）にもテキスト送信自体は動作するが、ファイル参照がエージェント側で認識されるかは各エージェントの仕様に依存する。
...
1. 環境変数 `$HERDR_WORKSPACE_ID` / `$HERDR_TAB_ID` で現在のワークスペースとタブを特定
2. `herdr agent list` を実行し、同一ワークスペース内のエージェントをフィルタ（Nvim自身のペインは除外）
3. 同一ワークスペースに0件なら `herdr pane split` + `herdr agent start` で自動起動（startコマンドがready待機を内包）
4. 同一タブにエージェントがいれば送信（1つなら直接送信、複数ならピッカーで選択）
5. 同一タブにおらず別タブにのみいる場合は、`vim.ui.select` で「このタブに新規起動」か「別タブのエージェントに送信」を選択
6. Selection/Buffer: `herdr pane send-text` でプロンプト欄にテキスト入力（末尾スペース付き）+ フォーカス移動
7. Prompt: `herdr agent prompt` でサブミット
8. 送信成功後、送信先のペインのタイトルと pane_id を通知
